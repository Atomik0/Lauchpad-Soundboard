...
function hasWineInstalled() {
  ...
  try {
    ...
    const winZipPath = path.join(__dirname, 'dist_exe', `Launchpad-Studio-Pro-v${nextVersion}-win32-x64.zip`);
    if (finalOutputDir && fs.existsSync(finalOutputDir)) {
      console.log(`[BUILD] Comprimiendo paquete completo Windows en ${path.basename(winZipPath)}...`);
      const parentDir = path.dirname(finalOutputDir);
      const folderName = path.basename(finalOutputDir);
      try {
        if (process.platform === 'win32') {
          try {
            execSync(`tar -a -c -f "${winZipPath}" -C "${parentDir}" "${folderName}"`, { stdio: 'inherit' });
          } catch {
            execSync(`powershell -NoProfile -Command "Compress-Archive -Path '${finalOutputDir}' -DestinationPath '${winZipPath}' -Force"`, { stdio: 'inherit' });
          }
        } else {
          execSync(`cd "${parentDir}" && zip -r -q "${winZipPath}" "${folderName}"`, { stdio: 'inherit' });
        }
        if (fs.existsSync(winZipPath)) {
          const stat = fs.statSync(winZipPath);
          console.log(`[BUILD] Paquete Windows ZIP generado con exito: ${path.basename(winZipPath)} (${(stat.size / (1024 * 1024)).toFixed(2)} MB)`);
        }
      } catch (zipErr) {
        console.warn('[BUILD] Aviso: No se pudo comprimir el paquete ZIP completo automaticamente:', zipErr.message);
      }
    }
  }
  catch (err) {
