  ...
  setTimeout(async () => {
    try {
      ...
      if (info && info.updateAvailable) {
        ...
        const cur = parseVer(info.currentVersion);
        const lat = parseVer(info.latestVersion);
        let isActuallyNewer = false;
        for (let i = 0; i < Math.max(cur.length, lat.length); i++) {
          if ((lat[i] || 0) > (cur[i] || 0)) { isActuallyNewer = true; break; }
...
function selectPad(index) {
  ...
  const isFnPad = (state.fnKeyEnabled !== false) && (index === (state.gridPadCount - 1));
  ...
  const isLastPad = (index === (state.gridPadCount - 1));
  let restoreFnBanner = document.getElementById('restore-fn-banner');
  if (isLastPad && !state.fnKeyEnabled) {
    if (!restoreFnBanner) {
      restoreFnBanner = document.createElement('div');
      restoreFnBanner.id = 'restore-fn-banner';
      restoreFnBanner.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:10px;background:rgba(168,85,247,0.12);border:1px solid rgba(168,85,247,0.35);border-radius:8px;padding:9px 12px;margin-bottom:10px;';
      restoreFnBanner.innerHTML = `
        <span style="font-size:11px;color:#c084fc;font-weight:600;">Este pad puede actuar como <strong>Tecla FN</strong> de capas.</span>
        <button id="btn-restore-fn" style="flex-shrink:0;background:rgba(168,85,247,0.25);border:1px solid #a855f7;border-radius:6px;color:#e9d5ff;font-size:11px;font-weight:700;padding:5px 12px;cursor:pointer;white-space:nowrap;">Restaurar FN</button>
      `;
      normalControls.insertBefore(restoreFnBanner, normalControls.firstChild);
      document.getElementById('btn-restore-fn').addEventListener('click', () => {
        state.fnKeyEnabled = true;
        const chk = document.getElementById('check-enable-fn-pad');
        if (chk) chk.checked = true;
        renderPads();
        selectPad(index);
        saveSoundboardConfigDebounced();
        showToast('Tecla FN restaurada', 'layers');
      });
    }
    restoreFnBanner.style.display = 'flex';
  } else if (restoreFnBanner) {
    restoreFnBanner.style.display = 'none';
  }
...
function triggerPad(index, isFromHardware = false) {
  ...
  if (typeof ipcRenderer !== 'undefined' && ipcRenderer) {
    ipcRenderer.send('mobile-broadcast', {
      type: 'trigger_light',
      payload: {
        row: pad.row,
        col: pad.col,
        colorHex: pad.color,
        effectType: effectToTrigger
      }
    });
  }
  ...
  if (typeof ipcRenderer !== 'undefined' && ipcRenderer) {
    ipcRenderer.send('mobile-broadcast', {
      type: 'pad_state_changed',
      payload: { padIndex: index, isPlaying: true }
    });
  }
...
function stopPad(index, stopDiscord = false) {
  ...
  if (typeof ipcRenderer !== 'undefined' && ipcRenderer) {
    ipcRenderer.send('mobile-broadcast', {
      type: 'pad_state_changed',
      payload: { padIndex: index, isPlaying: false }
    });
  }
...
async function syncAllPadsToHardware() {
  ...
  if (_isSyncingHardware) {
    // Si ya hay un sync en curso, esperar a que termine y re-intentar
    // para evitar que el W inicial deje el firmware sin los C subsiguientes.
    setTimeout(syncAllPadsToHardware, 50);
    return;
  }
...
async function restoreFromConfig(savedConfig) {
  ...
  // Solo deshabilitar FN si el respaldo lo marca explícitamente como false.
  // Respaldos viejos sin el campo, o con null/undefined, quedan habilitados por defecto.
  if (savedConfig.fnKeyEnabled === false) {
    state.fnKeyEnabled = false;
  } else {
    state.fnKeyEnabled = true;
  }
...
function initMobileRemoteBridge() {
  if (typeof ipcRenderer !== 'undefined' && ipcRenderer) {
    // Escuchar pulsacion de pad desde el telefono
    ipcRenderer.on('remote-pad-pressed', (event, { bankId, padIndex }) => {
      if (bankId && bankId !== state.currentBankId && typeof switchBank === 'function') {
        switchBank(bankId);
      }
      if (padIndex !== undefined && padIndex !== null && typeof triggerPad === 'function') {
        triggerPad(padIndex);
      }
    });

    // Escuchar cambio de banco desde el telefono
    ipcRenderer.on('remote-switch-bank', (event, { bankId }) => {
      if (bankId && typeof switchBank === 'function') {
        switchBank(bankId);
      }
    });

    // Escuchar parada de emergencia desde el telefono
    ipcRenderer.on('remote-stop-all', () => {
      if (typeof stopAllSounds === 'function') {
        stopAllSounds();
        showToast('Sonidos detenidos desde el Pad Móvil', 'stop');
      }
    });

    // Escuchar conteo de dispositivos moviles conectados
    ipcRenderer.on('mobile-client-count', (event, count) => {
      const badge = document.getElementById('mobile-remote-count-badge');
      const statusEl = document.getElementById('mobile-connected-status');
      if (badge) {
        badge.textContent = count;
        badge.style.display = count > 0 ? 'inline-flex' : 'none';
      }
      if (statusEl) {
        statusEl.textContent = `${count} activo${count === 1 ? '' : 's'}`;
      }
    });
  }

  // Interfaz de modal y codigo QR
  const btnMobileRemote = document.getElementById('btn-mobile-remote');
  const modalMobileRemote = document.getElementById('modal-mobile-remote');
  ...
  const btnCopyUrl = document.getElementById('btn-copy-mobile-url');

  if (btnMobileRemote && modalMobileRemote) {
    btnMobileRemote.addEventListener('click', async () => {
      if (typeof ipcRenderer !== 'undefined' && ipcRenderer) {
        try {
          const info = await ipcRenderer.invoke('get-mobile-remote-info');
          if (info && info.success) {
            if (qrImg) qrImg.src = info.qrDataUrl;
            if (urlInput) urlInput.value = info.url;
            const statusEl = document.getElementById('mobile-connected-status');
            if (statusEl) {
              statusEl.textContent = `${info.connectedCount} activo${info.connectedCount === 1 ? '' : 's'}`;
            }
          }
        } catch (err) {
          console.warn('Error obteniendo informacion del servidor movil:', err);
        }
      }
      modalMobileRemote.style.display = 'flex';
    });
  }

  if (btnCloseMobileModal && modalMobileRemote) {
    btnCloseMobileModal.addEventListener('click', () => {
      modalMobileRemote.style.display = 'none';
    });
    modalMobileRemote.addEventListener('click', (e) => {
      if (e.target === modalMobileRemote) {
        modalMobileRemote.style.display = 'none';
      }
    });
  }
  ...
  if (btnCopyUrl && urlInput) {
    btnCopyUrl.addEventListener('click', () => {
      if (navigator.clipboard && urlInput.value) {
