...
PUBLIC_REPO="git@github.com:Atomik0/Launchpad-Soundboard.git"
...
prepare_clean_public_staging() {
  ...
  rsync -av --exclude='.git' \
            ...
            --exclude='releases_para_subir' \
            "$REPO_DIR/" "$STAGING_DIR/" >/dev/null
...
if [ -n "$1" ]; then
  ...
  read -p "Numero de version a publicar [v$DEFAULT_NEXT]: " INPUT_VER
  NEW_VERSION="${INPUT_VER:-$DEFAULT_NEXT}"
  NEW_VERSION="${NEW_VERSION#v}"
fi

export BUILD_VERSION="$NEW_VERSION"

if [ "$NEW_VERSION" = "1.0.15" ]; then
  DEFAULT_TITLE="Launchpad Studio Pro v1.0.15 - Fusion Neon y Mejoras de FN"
  DEFAULT_NOTES="### Novedades y Mejoras en v1.0.15

#### Auto-Actualizador in-app
- Correccion de la ruta oficial del repositorio de GitHub para descargas y alertas de nuevas versiones.
- Validacion SemVer estricta para asegurar que solo alerte si la version remota es superior.

#### Pads y Capas
- Nuevo acceso rapido 'Restaurar FN' para recuperar la tecla de funcion si fue deshabilitada.
- Retrocompatibilidad asegurada con configuraciones previas (FN habilitada por defecto).

#### Hardware y Efectos LED (Firmware STM32)
- Modo visual 'Fusion Neon': la iluminacion de fondo modula el brillo del pad manteniendo el color asignado.
- Eliminacion del LED estatico fijo en reposo para la tecla FN.
- Prevencion de condiciones de carrera serie al cambiar rapidamente de banco o capa.

#### Seguridad y Distribucion
- Auditoria automatizada anti-fugas antes de publicar.
- Saneamiento y optimizacion de archivos base y reorganizacion modular de scripts."
else
  DEFAULT_TITLE="Launchpad Studio Pro v$NEW_VERSION"
  DEFAULT_NOTES="Actualizacion v$NEW_VERSION con mejoras de rendimiento, estabilidad y nuevas funciones."
fi

if [ -n "$2" ]; then
  RELEASE_TITLE="$2"
else
  read -p "Titulo del Release [$DEFAULT_TITLE]: " INPUT_TITLE
  RELEASE_TITLE="${INPUT_TITLE:-$DEFAULT_TITLE}"
fi
...
if [ -d "$APP_DIR/dist_exe/Launchpad Studio Pro-v$NEW_VERSION-win32-x64" ] && \
  ...
  if [ -f "$REPO_DIR/scripts/build/compilar_win.sh" ]; then
    bash "$REPO_DIR/scripts/build/compilar_win.sh"
  fi
fi
...
if [ -d "$APP_DIR/dist_linux/Launchpad Studio Pro-v$NEW_VERSION-linux-x64" ]; then
  ...
  if [ -f "$REPO_DIR/scripts/build/compilar_linux.sh" ]; then
    bash "$REPO_DIR/scripts/build/compilar_linux.sh"
  fi
fi
...
git config user.name "$(git -C "$REPO_DIR" config user.name || echo 'Atomik0')"
git config user.email "$(git -C "$REPO_DIR" config user.email || echo 'developer@soundboard.local')"
...
if command -v gh >/dev/null 2>&1; then
  ...
  gh release create "v$NEW_VERSION" "${RELEASE_ASSETS[@]}" \
    --repo "Atomik0/Launchpad-Soundboard" \
...
mkdir -p "$REPO_DIR/releases_para_subir"
for asset in "${RELEASE_ASSETS[@]}"; do
  cp -f "$asset" "$REPO_DIR/releases_para_subir/" 2>/dev/null || true
