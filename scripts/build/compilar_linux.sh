...
check_vps_credentials_before_build() {
  local CREDS_FILE="$REPO_DIR/vps_credentials.json"
...
if [ -f "$REPO_DIR/vps_credentials.json" ]; then
  node -e "
    const fs = require('fs');
    const c = JSON.parse(fs.readFileSync('$REPO_DIR/vps_credentials.json', 'utf8'));
    c.isPreconfigured = true;
    fs.writeFileSync('$APP_DIR/public/vps_default.json', JSON.stringify(c, null, 2), 'utf8');
  "
fi

echo "[3/4] Compilando ejecutable para Linux con conexion al VPS..."
...
if [ -n "$1" ]; then
  (cd "$APP_DIR" && node build_linux.js "$1")
else
...
LINUX_DIR="$APP_DIR/dist_linux/Launchpad Studio Pro-v$APP_VERSION-linux-x64"
LINUX_TAR="$APP_DIR/dist_linux/Launchpad-Studio-Pro-v$APP_VERSION-linux-x64.tar.gz"
LINUX_ZIP="$APP_DIR/dist_linux/Launchpad-Studio-Pro-v$APP_VERSION-linux-x64.zip"
UPDATE_ZIP="$APP_DIR/dist_linux/app-update-v$APP_VERSION.zip"

if [ -d "$LINUX_DIR" ] && [ ! -f "$LINUX_TAR" ]; then
  echo "[*] Comprimiendo paquete Linux en .tar.gz..."
  (cd "$APP_DIR/dist_linux" && tar -czf "Launchpad-Studio-Pro-v$APP_VERSION-linux-x64.tar.gz" "Launchpad Studio Pro-v$APP_VERSION-linux-x64" 2>/dev/null || true)
fi

if [ -d "$LINUX_DIR" ] && [ ! -f "$LINUX_ZIP" ]; then
  (cd "$APP_DIR/dist_linux" && zip -r -q "Launchpad-Studio-Pro-v$APP_VERSION-linux-x64.zip" "Launchpad Studio Pro-v$APP_VERSION-linux-x64" 2>/dev/null || true)
fi

mkdir -p "$REPO_DIR/releases_para_subir"
[ -f "$LINUX_TAR" ] && cp -f "$LINUX_TAR" "$REPO_DIR/releases_para_subir/" 2>/dev/null || true
[ -f "$LINUX_ZIP" ] && cp -f "$LINUX_ZIP" "$REPO_DIR/releases_para_subir/" 2>/dev/null || true
[ -f "$UPDATE_ZIP" ] && cp -f "$UPDATE_ZIP" "$REPO_DIR/releases_para_subir/" 2>/dev/null || true

echo "  Archivos listos para distribuir (v$APP_VERSION):"
[ -f "$LINUX_TAR" ] && echo "  - Paquete .tar.gz:      $LINUX_TAR"
[ -f "$LINUX_ZIP" ] && echo "  - Paquete .zip:         $LINUX_ZIP"
[ -f "$UPDATE_ZIP" ] && echo "  - Actualizador in-app:  $UPDATE_ZIP"
echo ""
...
echo "  - El binario principal ya cuenta con permisos de ejecucion."
echo ""
echo "  Para ejecutar en Linux:"
echo "  cd 'app/dist_linux/Launchpad Studio Pro-v$APP_VERSION-linux-x64' && ./'Launchpad Studio Pro'"
echo "========================================================"
