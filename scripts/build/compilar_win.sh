...
if [ -n "$1" ]; then
  (cd "$APP_DIR" && node build_exe.js "$1")
else
...
WIN_DIR="$APP_DIR/dist_exe/Launchpad Studio Pro-v$APP_VERSION-win32-x64"
WIN_ZIP="$APP_DIR/dist_exe/Launchpad-Studio-Pro-v$APP_VERSION-win32-x64.zip"
UPDATE_ZIP="$APP_DIR/dist_exe/app-update-v$APP_VERSION.zip"

if [ -d "$WIN_DIR" ] && [ ! -f "$WIN_ZIP" ]; then
  echo "[*] Comprimiendo paquete Windows en ZIP..."
  (cd "$APP_DIR/dist_exe" && zip -r -q "Launchpad-Studio-Pro-v$APP_VERSION-win32-x64.zip" "Launchpad Studio Pro-v$APP_VERSION-win32-x64" 2>/dev/null || true)
fi

mkdir -p "$REPO_DIR/releases_para_subir"
[ -f "$WIN_ZIP" ] && cp -f "$WIN_ZIP" "$REPO_DIR/releases_para_subir/" 2>/dev/null || true
...
[ -f "$WIN_ZIP" ] && echo "  - Paquete ZIP completo: $WIN_ZIP"
